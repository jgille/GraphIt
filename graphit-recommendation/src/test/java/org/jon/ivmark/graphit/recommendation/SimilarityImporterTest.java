...
public class SimilarityImporterTest {

    @Test(expected = IllegalArgumentException.class)
    public void testImportMissing() {
        SimilarityImporter.importSimilarities(new File(""), ';');
    }

    @Test
    public void testImport() {
        File file = ResourceUtils.resourceFile("fixtures/similarities.csv");
        List<Similarity> similarities = SimilarityImporter.importSimilarities(file, ';');
        assertThat(similarities, notNullValue());
        assertThat(similarities.size(), is(2));
    }
